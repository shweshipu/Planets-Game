...
export var zoom_strength = 0.8
# As distances between the camera and its target
var zoom_minimum = 5
var zoom_maximum = 50 
#added by shwesh
export var zoom_speed_reducer = 4.0
export var use_cpl = false
...
export var enable_pitch_limit = true  # up & down

#export var enable_updown_limit = true  # added by shwesh, limit up down movement?

export var enable_zoom_clamp = false #added by shwesh

export var enable_pitch_clamp = true #added by shwesh

# Limits as fraction of a quarter-circle = TAU/4
export(float, 0, 1, 0.005) var pitch_up_limit = 0.90
export(float, 0, 1, 0.005) var pitch_down_limit = 0.90
export(float, 0, 100, 0.05) var pitch_limit_strength = 10.0
...
func handle_mouse_input(event):
	if (not mouse_move_mode) and (event is InputEventMouseButton):
		if event.pressed and event.get_button_index() == BUTTON_MIDDLE:
	...
	if (mouse_move_mode) and (event is InputEventMouseMotion):
		add_inertia(event.relative * mouse_strength * 0.00005 * get_current_zoom()/10)#im not sure the getcurrent zoom even does anything? but eh
...
func process_zoom(delta):
	if zoom_enabled:
		...
		#added by shwesh, makes it so it'll scroll faster if its farther out.
		var cpl
		if use_cpl:
			cpl = get_current_zoom()
		else:
			cpl = zoom_speed_reducer
		
		if Input.is_action_just_released(action_zoom_in):
			add_zoom_inertia(zoo_i * zoo_s * cpl/zoom_speed_reducer)
		if Input.is_action_just_released(action_zoom_out):
			add_zoom_inertia(zoo_i * zoo_s * -1 * cpl/zoom_speed_reducer)
...
func process_zoom_inertia(delta):
	...
	var cpl = get_current_zoom()
	if abs(_zoomInertia) > zoomInertiaTreshold:
		if not enable_zoom_clamp: #added by shwesh, disable this if we clamping instead
			if cpl < zoom_minimum:
				if _zoomInertia > 0:
					_zoomInertia *= max(0, 1 - (1.333 * (zoom_minimum - cpl) / zoom_minimum))
				_zoomInertia = _zoomInertia - 0.1 * (zoom_minimum - cpl) / zoom_minimum
			if cpl > zoom_maximum:
				_zoomInertia += 0.09 * 1#exp((cpl - zoom_maximum) )#* 3 + 1) #* (cpl - zoom_maximum) * (cpl - zoom_maximum)
...
func apply_zoom(amount):
	if enable_zoom_clamp:
		amount = clamp_zoom(amount)
	translate(ZOOM_IN * amount)

func clamp_zoom(amount): #added by shwesh, makes it stop at min zoom instead of doing a bounce back
	if enable_zoom_clamp:
		var cpl = get_current_zoom()
		if cpl < zoom_minimum:
			if amount > 0:
...
func apply_constraints(on_transform):
	if enable_pitch_clamp:
		on_transform = apply_pitch_clamp(on_transform)
	elif enable_pitch_limit:
		on_transform = apply_pitch_constraint(on_transform)
#	if enable_updown_limit:
#		on_transform = apply_updown_constraint(on_transform)
	return on_transform
...
#		_dragInertia *= 0.618
	...
	return on_transform
func apply_pitch_clamp(on_transform):
	var eulers = on_transform.basis.get_euler()
	...
	"""
	idk how to make it do it better and not have it wait a frame so i just redid the calcs from apply_rotation_from_tangent() again.
	its just cuz i dont know hot to do those euler tests without putting it through the calcs
	"""
	var tangent = _dragInertia * inertia_strength
...
#added by shwesh
func start_position():
	self.transform.origin = Vector3(0,0,zoom_minimum)#added by shwesh
	var rotationQuat = get_parent().transform.basis.get_rotation_quat()
	look_at(get_parent().transform.origin,rotationQuat.xform(Vector3.UP))
